...
QUnit.test('exists in window', function (assert) {
	assert.ok('linkify' in window);
});

QUnit.test('contains find function', function (assert) {
	assert.ok('find' in linkify);
	assert.equal(typeof linkify.find, 'function');
});

QUnit.test('contains test function', function (assert) {
	assert.ok('test' in linkify);
	assert.equal(typeof linkify.test, 'function');
});

QUnit.test('contains tokenize function', function (assert) {
	assert.ok('tokenize' in linkify);
	assert.equal(typeof linkify.tokenize, 'function');
});

QUnit.test('contains an options object', function (assert) {
	assert.ok('options' in linkify);
	assert.equal(typeof linkify.options, 'object');
});

QUnit.test('contains a parser object', function (assert) {
	assert.ok('parser' in linkify);
	assert.equal(typeof linkify.parser, 'object');
});

QUnit.test('contains a scanner object', function (assert) {
	assert.ok('scanner' in linkify);
	assert.equal(typeof linkify.scanner, 'object');
});
...
QUnit.test('linkify.find correctly finds a URL', function (assert) {
	var result = linkify.find('The url is github.com');
...
QUnit.test('linkify.find correctly finds an email address', function (assert) {
	var result = linkify.find('The url is github.com and the email is test@example.com', 'email');
	assert.equal(result.length, 1);
...
QUnit.test('contains normalize function', function (assert) {
	assert.ok('normalize' in linkify.options);
	assert.equal(typeof linkify.options.normalize, 'function');
});

QUnit.test('contains resolve function', function (assert) {
	assert.ok('resolve' in linkify.options);
	assert.equal(typeof linkify.options.resolve, 'function');
});
...
QUnit.test('returns in the hash of default options when given an empty object', function (assert) {
	var result = linkify.options.normalize({});
...
QUnit.test('results in the value given when called with a non-function', function (assert) {
	...
	assert.equal(linkify.options.resolve(result0), result0);
	assert.equal(linkify.options.resolve(result1), result1);
	assert.equal(linkify.options.resolve(result2), result2);
	assert.equal(linkify.options.resolve(result3), result3);
});
...
QUnit.test('Results the results of the function when called with a function and arguments', function (assert) {
	...
	assert.equal(
		linkify.options.resolve(concat, 'testone', 'testtwo'), 'testonetesttwo'
	);
});
...
QUnit.test('finds valid hashtags', function (assert) {
	var result = linkify.find('#urls are #awesome2015');
...
QUnit.test('linkifyElement exists', function (assert) {
	assert.ok('linkifyElement' in window);
	assert.equal(typeof linkifyElement, 'function');
});
...
QUnit.test('linkifyHtml exists', function (assert) {
	assert.ok('linkifyHtml' in window);
	assert.equal(typeof linkifyHtml, 'function');
});

QUnit.test('works with default options', function (assert) {
	var result = linkifyHtml(originalHtml);
...
QUnit.test('linkifyStr exists', function (assert) {
	assert.ok('linkifyStr' in window);
	assert.equal(typeof linkifyStr, 'function');
});

QUnit.test('works with default options', function (assert) {
	var result = linkifyStr('google.ca and me@gmail.com');
