...
export function init(customSchemes = []) {
	...
	const Num = tr(Start, re.DIGIT, [tk.NUM, [fsm.numeric]], groups);
	tr(Num, re.DIGIT, Num);

	// State which emits a word token
	const Word = tr(Start, re.ASCII_LETTER, [tk.WORD, [fsm.ascii]], groups);
	tr(Word, re.ASCII_LETTER, Word);

	// Same as previous, but specific to non-fsm.ascii alphabet words
	const UWord = tr(Start, re.LETTER, [tk.UWORD, [fsm.alpha]], groups);
	...
	const Ws = tr(Start, re.SPACE, [tk.WS, [fsm.whitespace]], groups);
	tt(Start, NL, [tk.NL, [fsm.whitespace]], groups);
	tt(Ws, NL); // non-accepting state to avoid mixing whitespaces
	tr(Ws, re.SPACE, Ws);

	// Emoji tokens. They are not grouped by the scanner except in cases where a
	// zero-width joiner is present
	const Emoji = tr(Start, re.EMOJI, [tk.EMOJI, [fsm.emoji]], groups);
	...
	for (let i = 0; i < tlds.length; i++) {
		ts(Start, tlds[i], [tk.TLD, [fsm.tld, fsm.ascii]], groups);
	}
	for (let i = 0; i < utlds.length; i++) {
		ts(Start, utlds[i], [tk.UTLD, [fsm.utld, fsm.alpha]], groups);
	}

	// Collect the states generated by different protocols. NOTE: If any new TLDs
	// get added that are also protocols, set the token to be the same as the
	// protocol to ensure parsing works as expected.
	ts(Start, 'file', [tk.SCHEME, [fsm.scheme, fsm.ascii]], groups);
	ts(Start, 'mailto', [tk.SCHEME, [fsm.scheme, fsm.ascii]], groups);
	ts(Start, 'http', [tk.SLASH_SCHEME, [fsm.slashscheme, fsm.ascii]], groups);
	ts(Start, 'https', [tk.SLASH_SCHEME, [fsm.slashscheme, fsm.ascii]], groups);
	ts(Start, 'ftp', [tk.SLASH_SCHEME, [fsm.slashscheme, fsm.ascii]], groups);
	ts(Start, 'ftps', [tk.SLASH_SCHEME, [fsm.slashscheme, fsm.ascii]], groups);
	...
	for (let i = 0; i < customSchemes.length; i++) {
		...
		ts(Start, sch, [sch, c], groups);
	}

	// Localhost token
	ts(Start, 'localhost', [tk.LOCALHOST, [fsm.ascii]], groups);
