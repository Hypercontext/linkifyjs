...
const T_DOMAIN		= TOKENS.DOMAIN;
const T_LOCALHOST	= TOKENS.LOCALHOST;
const T_NUM			= TOKENS.NUM;
const T_PROTOCOL	= TOKENS.PROTOCOL;
const T_TLD			= TOKENS.TLD;
const T_WS			= TOKENS.WS;

// Frequently used states
const S_START			= makeState();
const S_NUM				= makeState(T_NUM);
const S_DOMAIN			= makeState(T_DOMAIN);
const S_DOMAIN_HYPHEN	= makeState(); // domain followed by 1 or more hyphen characters
const S_WS				= makeState(T_WS);
...
let partialProtocolFileStates	= stateify('file', S_START, T_DOMAIN, T_DOMAIN);
let partialProtocolFtpStates	= stateify('ftp', S_START, T_DOMAIN, T_DOMAIN);
let partialProtocolHttpStates	= stateify('http', S_START, T_DOMAIN, T_DOMAIN);
...
let S_PROTOCOL_SECURE	= makeState(T_DOMAIN);
let S_FULL_PROTOCOL		= makeState(T_PROTOCOL); // Full protocol ends with COLON
